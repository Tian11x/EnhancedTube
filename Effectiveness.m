...
if strcmp(Dataset, 'Seattle')
    ...
    beta = 0.98;
    save_path = 'experiment_uniform/finalResult/EffectiveSeattle.mat';
    load('experiment_uniform/finalResult/EffectiveSeattle.mat');
elseif strcmp(Dataset, 'PlanetLab')
    data_path = 'experiment_uniform/data/PlanetLab490';
    load(data_path);  % load data
    Tensor = PlanetLab;
    Tensor = Tensor ./ max(max(max(Tensor))); 
    Target = Tensor(1:120,1:120,1:18);
    rank = 12;
    u = 12;
    beta = 0.999;
    % loading alpha value
    load('experiment_uniform/finalResult/AlphaTrainPlanetLab.mat');
    % load('experiment_uniform/finalResult/EffectivePlanetLab120.mat');
    save_path = 'experiment_uniform/finalResult/EffectivePlanetLab.mat';
end

[m, n, k] = size(Target);
alpha = out_alpha;

% 采样率最小间隔并初始化结构体用于存储结果
sample_step = 0.1;
% ours = init_struct();
...
for p = 0.1:sample_step:0.9
    ...
    t = tic;
    [~, samp_Omega, scheCount] = MACDschedule( Target, p, rank, 0.1, 'tubeOn-cos', u, alpha, beta);
    sample_time = toc(t); % 统计采样时间
    t = tic;
    Recovered = tensor_ADMM(Target, samp_Omega);
    recover_time = toc(t); % 统计恢复时间
...
%     t = tic;
%     [Recovered,IDX_A]=tensor_adaptive_sample_fiber(Target,m*n*p,0.3,rank);
%     time = toc(t);
